...
func _ready():
	...
	var random = randi_range(10, 35)
	for i in range(random):
		var new_paper = paper_prefab.instantiate()
		paper_container.add_child(new_paper)    
		if GameManager.current_paper_work == "Adams":
			new_paper.get_node("TextureRect").modulate = Color.ORANGE        
		elif GameManager.current_paper_work == "King":
			new_paper.get_node("TextureRect").modulate = Color.SLATE_GRAY        
		elif GameManager.current_paper_work == "Allen":
			new_paper.get_node("TextureRect").modulate = Color.PALE_GREEN 
		elif GameManager.current_paper_work == "Scott":
			new_paper.get_node("TextureRect").modulate = Color.SADDLE_BROWN
		elif GameManager.current_paper_work == "Black":
			new_paper.get_node("TextureRect").modulate = Color.PINK
